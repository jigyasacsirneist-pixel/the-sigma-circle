  ...
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <svg width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
          <defs>
            <pattern id="grid" width="40" height="40" patternUnits="userSpaceOnUse">
              <path d="M 40 0 L 0 0 0 40" fill="none" stroke="currentColor" strokeWidth="0.5" className="text-science-gold" />
            </pattern>
            <pattern id="grid-large" width="200" height="200" patternUnits="userSpaceOnUse">
              <path d="M 200 0 L 0 0 0 200" fill="none" stroke="currentColor" strokeWidth="1" className="text-science-gold" />
            </pattern>
          </defs>
      ...
      {mathSymbols.map((item, index) => (
        <div
          key={index}
          className={`absolute ${item.size} font-serif text-primary-foreground/10 animate-float-symbol`}
          style={{
            left: item.left,
            top: item.top,
      ...
      {geometricShapes.map((shape, index) => (
        ...
        >
          {shape.type === "triangle" && (
            <svg width={shape.size} height={shape.size} viewBox="0 0 100 100" className="text-science-gold/10">
              <polygon points="50,10 90,90 10,90" fill="none" stroke="currentColor" strokeWidth="2" />
            </svg>
          )}
          {shape.type === "circle" && (
            <svg width={shape.size} height={shape.size} viewBox="0 0 100 100" className="text-primary-foreground/10">
              <circle cx="50" cy="50" r="40" fill="none" stroke="currentColor" strokeWidth="2" />
            </svg>
          )}
          {shape.type === "sine" && (
            <svg width={shape.width} height="40" viewBox="0 0 100 40" className="text-science-gold/15">
